...
router.put('/update/lessonByID', (req, res) => {
  ...
  let updateQuery = db_query.updateQuery("lessons", column, "id", false)
...
router.put('/updateAll/lessonByID', (req, res) => {
  const { title, duration, media_type, media_value, subject_id, condition, conditionValue } = req.body
  let updateQuery = db_query.updateQuery("lessons", "?", condition, true)
  console.log(updateQuery);
  console.log(JSON.stringify(req.body, null, 2) + "Boody")
